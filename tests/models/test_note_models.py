...
def test_empty_title_raise_value_error():
    with pytest.raises(ValueError, match="Title must by non-empty string."):
        _ = Note(
            title="",
...
def test_empty_tags_raise_value_error():
    with pytest.raises(ValueError, match=re.escape("Tags: set() must by non-empty set.")):
        _ = Note(
            title="Note title",
            content="Note content",
            tags=set(),
...
def test_empty_content_rise_value_error() -> None:
    with pytest.raises(ValueError, match="Content must by non-empty string."):
